...
const Conversation = ({conversation,isLoading, error, chatRef,isSuccess}) => {

  const [currentDateTime, setCurrentDateTime] = useState(format(new Date(), 'MMM dd, HH:mm a'));
  ...
  return (
    <div className="w-full p-4 h-[60vh] overflow-y-auto" >
     ...
     {error ? <p className="text-red-400 text-xs">Error: Failed to Loading</p> : null }

      {
       isSuccess && conversation.length > 0 ?  conversation.map((item,index) => {
